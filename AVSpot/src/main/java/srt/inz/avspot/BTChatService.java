...
public class BTChatService {
	...
	private synchronized void setState(int state) {
		...
		mHandler.obtainMessage(BTchat.MESSAGE_STATE_CHANGE, state, -1)
				.sendToTarget();
	}
	...
	private class ConnectedThread extends Thread {
		...
		public void run() {
			...
			while (true) {
				try {
					...
					mHandler.obtainMessage(BTchat.MESSAGE_READ, bytes,
		...
		public void write(byte[] buffer) {
			try {
				...
				mHandler.obtainMessage(BTchat.MESSAGE_WRITE, -1, -1,
						buffer).sendToTarget();
			} catch (IOException e) {
